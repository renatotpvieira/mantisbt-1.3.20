...
<body>
	...
	if ($t_config_exists) {
		if (0 == $t_install_state) {
			...
			print_test(
				'Checking Database connection settings exist',
				$t_db_conn_exists,
				true,
			...
			print_test(
				...
				db_check_database_support($f_db_type),
				true,
			...
			if ($f_db_type == 'mssql') {
				print_test(
					'Checking PHP support for Microsoft SQL Server driver',
	...
	if (2 == $t_install_state) {
		...
		print_test(
			'Checking ADOdb Library version is at least ' . DB_MIN_VERSION_ADODB,
		...
		<form method='POST'>
			...
			<table width="100%" cellpadding="10" cellspacing="1">
				...
				<tr>
					<td bgcolor="#e8e8e8" colspan="2">
						<span class="title">
							<?php echo ($g_database_upgrade ? 'Upgrade Options' : 'Installation Options'), ($g_failed ? ': Checks Failed ' : '')
							?>
						</span>
					</td>
				</tr>
					...
					if ($f_log_queries) {
						if ($t_sql) {
							foreach ($t_sqlarray as $t_sql) {
								...
								if (gettype($t_sql) != 'string') {
									print_r($t_sql);
									$t_sql = 'Convertida: ' . (string)$t_sql;
								}
...
*/
	...
	?>
		<table width="100%" cellpadding="10" cellspacing="1">
			...
			if ($t_crypto_master_salt === null) {
				print_test(
					'Setting Cryptographic salt in config file',
					false,
					false,
